...
public:
  ...
  //my implementation//
  // map<uint32_t, map<uint32_t, pair<uint32_t, uint32_t> > > routing_td;
  UUID global_timer;
  map < uint32_t, uint32_t > routing_td; //<source_ip, metric>
  // set < uint32_t > neighbors;
  vector < uint32_t > neighbors;
  vector< uint32_t > links;
  map< uint32_t, uint32_t > my_links; //{key: (peerside)ip, key: (myside)ip}
